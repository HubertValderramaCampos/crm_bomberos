...
async function getData() {
  ...
  }>(`SELECT id, fecha::text, entidad, tipo_donacion FROM donacion ORDER BY fecha DESC`);
  return rows;
}
...
export default async function DonacionesPage() {
  ...
  return (
    <div className="space-y-4 pb-6">
      <PageHeader icon={Gift} title="Donaciones" subtitle={`${rows.length} registros`} />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  {["Fecha", "Entidad", "Tipo de Donación"].map(h => (
              ...
              <tbody className="divide-y divide-gray-100">
                {rows.map(r => (
                  <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3.5 font-medium text-gray-900 max-w-[220px]">
                      <p className="truncate">{r.entidad}</p>
                    </td>
                    <td className="px-5 py-3.5 text-xs text-gray-700">{r.tipo_donacion}</td>
                  </tr>
                ))}
              </tbody>
            </table>
